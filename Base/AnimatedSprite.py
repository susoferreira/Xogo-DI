...
class AnimatedSprite():
    ...
    def __init__(self,sprite_route: str, animation_delay:int = 1,animate:bool =True,scale: float = 1,pos:  Tuple[int, int] = (0,0)):
        ...
        self.image:pygame.Surface = pygame.image.load(self.get_descriptor()["default_image"])
        self.rect = self.image.get_rect()
        self.size=self.image.get_bounding_rect().size
        print("Tamaño original:",self.size)
        ...
        for animation in self.get_descriptor()["animations"]:
            if animation["name"] == self.animation_type:
                for frame in animation["frames"]:
                    ...
                    img = pygame.transform.scale(img,(int(self.image.get_width()*scale),int(self.image.get_height()*scale)))
                    imgs.append(img)
